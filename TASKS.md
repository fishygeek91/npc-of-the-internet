...
- **T7.1 ⬜ IPFS SoulStore** (helia impl of SoulStore; pinning strategy) — Deps: T6.2
  ...
  - **osp/0.2 schema + tombstones ✅** (text-off-chain blob refs; tombstone type; migration vectors; runtime side-blob write/compose/Atlas) — Deps: Gate E1 — Agent: Cursor Grok 4.5 Maestro, 2026-08-09 — Issue: #119
    - Notes: PR1 schema landed. PR2: SoulStore `put/get/deleteSideBlob`, `eraseSideBlob`+tombstone, runtime writes `osp/0.2` (`RUNTIME_OSP_SPEC`), compose visible `[memory erased: …]` marker, Atlas journal_cid resolve. `createRecord` default remains `osp/0.1` for vectors; Ghost pins V02. Next: T7.1d.
