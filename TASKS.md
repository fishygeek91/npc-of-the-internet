...
- Notes: Agent: Cursor Grok 4.5 Maestro, 2026-08-08. Session phases `live|departing|departed`; transcript destroyed after first read with in-memory retry cache; commit rebinds/re-cosigns when head moves mid-loop (flag TOCTOU). No RejectedBody `shard_id` schema change — host_rejected batch skip is all-or-nothing per residency.
