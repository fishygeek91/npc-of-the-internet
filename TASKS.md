...
- **T7.1 ⬜ IPFS SoulStore** (helia impl of SoulStore; pinning strategy) — Deps: T6.2
  ...
  - **osp/0.2 schema + tombstones ⏳** (text-off-chain blob refs; tombstone type; migration vectors; runtime blob path in follow-up PR) — Deps: Gate E1 — Agent: Cursor Grok 4.5 Maestro, 2026-08-09 — Issue: #119
    - Notes: PR1 (this branch): dual-version Zod/`records.md`, side-blob helpers, tombstone type + closed reason enum, migration/tombstone vectors. Ghost still defaults `OSP_SPEC=osp/0.1`. PR2: SoulStore side blobs + compose/Distiller/Atlas.
