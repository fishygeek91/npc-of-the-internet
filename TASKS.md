...
### Bug #65 ✅ door-discord: verify cosign before review-gate Discord posts (Cursor Grok 4.5 Maestro, 2026-07-29)
- Deps: T4.2
...
- Notes: Agent: Cursor Grok 4.5 Maestro, 2026-08-08. age (not GPG) for key backup; Door health via GET :9090 (404 OK); FP corpus documents current fail categories (T7.4 tighten later); backup.ok age healthcheck closes #63 deferral. Review follow-up: preflight probes `$BACKUP_RCLONE_REMOTE` via `lsjson` (bucket-scoped B2 keys); key-backup-drill live mode (`NPC_KEY_DRILL_LIVE=1` / `AGE_IDENTITY_PATH`) cmps remote latest against host keys.
...
- **T7.1 ✅ IPFS SoulStore** (blockstore-fs L1; dual-write; pin manifest/CAR; outbound replication) — Deps: T6.2 — Issue: #55
  ...
  - **T7.1c ✅ Pin manifest + CAR + CLI** (manifest build/sign/verify; export-car / manifest / verify --from-ipfs; round-trip) — Deps: T7.1b — Agent: Cursor Grok 4.5 Maestro, 2026-08-09 — Issue: #117
    - Notes: `@ipld/dag-json` + `@ipld/car`; pin-manifest via seq-index (not iterate); CAR round-trip; invalid-vector injection; CLI with injectable gateway fetcher. Next: T7.1d (Phase D) after #83 + #119.
  - **Gate E1 ✅ Privacy & erasure policy** (option (b) text-off-chain; privacy.md; charter/whitepaper/announcement; GitHub privacy contact) — Deps: T6.2 — Agent: Cursor Grok 4.5 Maestro, 2026-08-09 — Issue: #83
...
- **T7.11 ✅ Multi-provider Brain** (OpenAI-compatible impl + env provider config; usage on results; OpenRouter US allowlist) — Deps: T2.1 — Issue: #73
  - Notes: Agent: Cursor Grok 4.5 Maestro, 2026-08-18. `OpenAICompatBrain` (native fetch, no openai SDK); `NPC_BRAIN_PROVIDER=anthropic|openai-compat|fake`; `complete()` returns `{ text, usage }`; OpenRouter requires non-empty `NPC_BRAIN_PROVIDER_ALLOWLIST` sent as `provider.only`. Ghost example: OpenRouter + `deepseek/deepseek-v4-flash` + `deepinfra,gmicloud,venice,digitalocean,parasail,azure` (allowlist must be derived from the model's live `/endpoints` list — see ops/TROUBLESHOOTING.md; the original `fireworks,together,deepinfra` intersected to DeepInfra only and caused a total 429 outage at first contact). Next: T7.12 Treasury-lite (#74) can wrap usage.
...
- **Hotfix ✅ atlas-api Trivy republish** (root `fast-uri` override `4.1.2` → `>=4.1.3`; v0.3.2) — Deps: T7.11 — Issue: #147, PR: #148
  - Notes: Agent: Claude, 2026-09-20 (operator-authorized). The v0.3.1 atlas-api docker leg failed the Trivy gate on the previously-pinned override itself; other images published. Lesson: prefer `>=` floors over `=` pins for security overrides. First release fully on published images: v0.3.2.
- **Feature ✅ Selective attention + reactions + live transcript** (door/0.1 additive `session.reactions` / `session.addressing`; `Session.observe` speak/react/silent with coalescing + floor guard; `NPC_ATTENTION_MODE`; in-memory `ResidencyTranscript` for depart distillation — WHITEPAPER §3.2) — Deps: T2.4, T4.1, T7.11
  - Notes: Agent: Claude, 2026-09-26 (operator-authorized implementation). Triggered by live Discord play: the Wanderer answered every message and could only post text. Found along the way: Discord `@name` text trips the immune `pii.handle` screen and silently drops the whole message — the Door now renders mentions as plain names. The daemon still has no production depart trigger, so the transcript is captured but nothing distills it until an operator depart/move path exists (follow-up). Legacy `handleInbound` kept for `NPC_ATTENTION_MODE=always` and existing tests.
- **Ops ⬜ Automate release tagging** (version-packages merge should tag `v*` itself; today's manual `git tag && push` step is undocumented and was missed twice) — Deps: none — Issue: #150
